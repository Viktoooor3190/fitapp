...
import { 
  ...
  Clock, CheckCircle, 
  Dumbbell, Apple, UserCheck, Edit, Plus, Loader, Pencil, Loader2
} from 'lucide-react';
...
import PlanGenerationBadge from '../../../components/fitness/PlanGenerationBadge';
import { 
...
interface ClientDetails {
  ...
  plansAutoGenerated?: boolean;
  plansAutoGeneratedAt?: Date | null;
  plansAutoGeneratedDate?: string | null;
}
...
const ClientDetailsPage = () => {
  ...
  const [autoGenerationInfo, setAutoGenerationInfo] = useState({
    isAutoGenerated: false,
    generatedAt: null as Date | null,
    generatedDate: null as string | null
  });
  const [isAutoGenerating, setIsAutoGenerating] = useState(false);
  const [autoGenerationDisabled, setAutoGenerationDisabled] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchClientData = async () => {
      ...
      try {
        ...
        // Check if auto-generation is disabled for this client
        setAutoGenerationDisabled(clientData.disableAutoGeneration || false);
        ...
        // Get auto-generation info
        const profileRef = doc(db, 'profiles', clientId);
        const profileDoc = await getDoc(profileRef);
        
        if (profileDoc.exists()) {
          const profileData = profileDoc.data();
          setAutoGenerationInfo({
            isAutoGenerated: profileData.plansAutoGenerated || false,
            generatedAt: profileData.plansAutoGeneratedAt ? new Date(profileData.plansAutoGeneratedAt.toDate()) : null,
            generatedDate: profileData.plansAutoGeneratedDate || null
          });
        } else {
          // Try to get the data from users collection instead
          const userProfileRef = doc(db, 'users', clientId);
          const userProfileDoc = await getDoc(userProfileRef);
          
          if (userProfileDoc.exists()) {
            const userData = userProfileDoc.data();
            setAutoGenerationInfo({
              isAutoGenerated: userData.plansAutoGenerated || false,
              generatedAt: userData.plansAutoGeneratedAt ? new Date(userData.plansAutoGeneratedAt.toDate()) : null,
              generatedDate: userData.plansAutoGeneratedDate || null
            });
            
            // Also check if auto-generation is disabled
            setAutoGenerationDisabled(userData.disableAutoGeneration || false);
          }
        }
        ...
        // Check if we should auto-generate plans for this client
  ...
  const checkAndTriggerAutoGeneration = async () => {
    // Skip if auto-generation is already happening or disabled
    if (isAutoGenerating || autoGenerationDisabled) return;
    
    try {
      // First, check if plans have already been auto-generated
      const profileRef = doc(db, 'profiles', clientId!);
      const profileDoc = await getDoc(profileRef);
      
      // If auto-generation flag exists and is true, skip
      if (profileDoc.exists() && profileDoc.data().plansAutoGenerated) {
        console.log('Plans have already been auto-generated for this client');
        return;
      }
      
      // Check if the client has completed onboarding
      // We get the client data directly from the clients collection
      const clientRef = doc(db, 'clients', clientId!);
      const clientDoc = await getDoc(clientRef);
      
      // Only proceed if onboarding has been completed
      if (!clientDoc.exists() || !clientDoc.data().onboardingCompleted) {
        console.log('Client has not completed onboarding yet');
        return;
      }
      
      // Check whether auto-generation is disabled for this client
      if (clientDoc.data().disableAutoGeneration) {
        setAutoGenerationDisabled(true);
        return;
      }
      
      // All checks passed, start auto-generation
      ...
      // Extract client profile data from onboardingAnswers to use for plan generation
      const onboardingAnswers = clientDoc.data().onboardingAnswers || {};
      
      console.log('Using client onboarding data for plan generation:', onboardingAnswers);
      
      // Get Firebase Functions
      const functions = getFunctions();
      
      // Use httpsCallable instead of direct HTTP requests
      const generateWorkoutPlanFunction = httpsCallable(functions, 'aiGenerateWorkoutPlan');
      const generateNutritionPlanFunction = httpsCallable(functions, 'aiGenerateNutritionPlan');
      
      // Generate workout plan
      const workoutResponse = await generateWorkoutPlanFunction({
        userId: clientId,
        date: dayAfterTomorrowStr
      });
      
      // Generate nutrition plan
      const nutritionResponse = await generateNutritionPlanFunction({
        userId: clientId,
        date: dayAfterTomorrowStr
      });
      
      // Update the profile to mark auto-generation as complete
      // Note: Using the same profileRef from above, not redeclaring
      if (profileDoc.exists()) {
        await updateDoc(profileRef, {
          plansAutoGenerated: true,
          plansAutoGeneratedAt: new Date(),
          plansAutoGeneratedDate: dayAfterTomorrowStr
        });
      } else {
      ...
      setAutoGenerationInfo({
        ...
        generatedDate: dayAfterTomorrowStr
      });
      ...
      toast({
        title: "Success!",
        description: "AI plans have been automatically generated for this client.",
      });
    } catch (error) {
      console.error('Error auto-generating plans:', error);
      toast({
        title: "Error",
        description: "Failed to auto-generate plans. Please try again manually.",
        variant: "destructive"
      });
    } finally {
          ...
          <div className="border-t border-gray-700 p-4 bg-gray-900/80">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-white flex items-center">
                ...
                {autoGenerationInfo.isAutoGenerated && 
                  autoGenerationInfo.generatedDate === format(new Date(selectedDate), 'yyyy-MM-dd') && (
                  <div className="ml-2">
                    <PlanGenerationBadge 
                      isAutoGenerated={true}
                      autoGeneratedAt={autoGenerationInfo.generatedAt}
                      generatedDate={autoGenerationInfo.generatedDate}
                    />
                  </div>
                )}
              </h3>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center space-x-4 mb-6">
              ...
              <div>
                ...
                {/* Auto-generation indicator and toggle */}
                {isAutoGenerating && (
                  <div className="mt-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-200 dark:border-blue-800">
                    <div className="flex items-center space-x-2">
                      ...
                      <p className="text-sm text-blue-700 dark:text-blue-300">
                        Generating AI plans for this client...
                      </p>
                    </div>
                    <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                      This may take a minute. Plans will be generated for the day after tomorrow.
                    </p>
                  </div>
                )}

                {/* Toggle to disable auto-generation */}
                <div className="mt-4 flex items-center justify-between">
                  <div className="space-y-0.5">
                    <div className="text-sm font-medium text-gray-900 dark:text-white">Auto-generate AI plans</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      Automatically generate plans when viewing this client
                    </div>
                  </div>
                  <Switch 
                    checked={!autoGenerationDisabled}
                    onCheckedChange={async (enabled) => {
                      // Update the client's document
                      try {
                        const clientRef = doc(db, 'clients', clientId!);
                        await updateDoc(clientRef, {
                          disableAutoGeneration: !enabled
                        });
                        setAutoGenerationDisabled(!enabled);
                        
                        toast({
                          title: enabled ? "Auto-generation enabled" : "Auto-generation disabled",
                          description: enabled 
                            ? "AI plans will be generated automatically" 
                            : "AI plans will not be generated automatically",
                        });
                      } catch (error) {
                        console.error('Error updating auto-generation setting:', error);
                        toast({
                          title: "Error",
                          description: "Failed to update auto-generation setting",
                          variant: "destructive"
                        });
                      }
                    }}
